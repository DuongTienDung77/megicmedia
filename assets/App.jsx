...
// Globals are loaded from CDNs and the main.js bootstrap script.
const {
  useState,
  useEffect,
  createContext,
...
const ApiKeyModal = ({ isOpen, onClose }) => {
    ...
    const getStatusMessage = () => {
        switch (checkStatus) {
            case 'checking': return React.createElement("p", { className: "text-sm text-yellow-500 mt-2" }, t('checkingKey'));
            case 'valid': return React.createElement("p", { className: "text-sm text-green-500 mt-2" }, t('keyValid'));
            case 'invalid': return React.createElement("p", { className: "text-sm text-red-500 mt-2" }, t('keyInvalid'));
            default: return null;
        }
    };

    return (
        React.createElement("div", { className: "fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4 animate-fade-in", role: "dialog", "aria-modal": "true" },
            React.createElement("div", { ref: modalRef, className: "bg-light-surface dark:bg-dark-surface p-6 rounded-2xl shadow-xl w-full max-w-md border border-light-border dark:border-dark-border" },
                React.createElement("h2", { className: "text-xl font-bold mb-2 text-light-text dark:text-dark-text" }, t('enterApiKey')),
                React.createElement("p", { className: "text-sm text-gray-500 dark:text-gray-400 mb-4" }, t('apiKeyInstructions'), " ", React.createElement("a", { href: "https://aistudio.google.com/app/apikey", target: "_blank", rel: "noopener noreferrer", className: "text-light-primary dark:text-dark-primary underline" }, t('getApiKey'))),
                ...
                React.createElement("div", { className: "min-h-[24px]" },
                    getStatusMessage()
                ),

                React.createElement("div", { className: "grid grid-cols-3 gap-2 mt-2" },
                    React.createElement("button", { onClick: handleCheckKey, className: "w-full bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500 text-light-text dark:text-dark-text font-semibold py-2 px-4 rounded-lg transition-colors" }, t('checkKey')),
                    React.createElement("button", { onClick: handleSave, className: "w-full btn-primary text-white font-semibold py-2 px-4 rounded-lg" }, t('saveKey')),
                    React.createElement("button", { onClick: handleDelete, className: "w-full bg-gray-200 dark:bg-gray-600 hover:bg-gray-300 dark:hover:bg-gray-500 text-light-text dark:text-dark-text font-semibold py-2 px-4 rounded-lg transition-colors" }, t('deleteKey'))
                ),
                React.createElement("div", { className: "mt-2" },
                     React.createElement("button", { onClick: onClose, className: "w-auto bg-transparent hover:bg-gray-200 dark:hover:bg-gray-600 text-light-text dark:text-dark-text font-semibold py-2 px-4 rounded-lg transition-colors" }, t('cancel'))
                )
            )
        )
    );
...
const Header = ({ onApiKeyClick }) => {
  ...
  return (
    React.createElement("header", { className: "bg-light-surface dark:bg-dark-surface shadow-md p-4 flex justify-between items-center border-b border-light-border dark:border-dark-border" },
      React.createElement("h1", { className: "text-xl font-bold text-light-text dark:text-dark-text" }, "\u2728 ", t('appName')),
      React.createElement("div", { className: "flex items-center space-x-2 md:space-x-4" },
        React.createElement("button", { onClick: toggleTheme, title: t('toggleTheme'), className: "text-light-text dark:text-dark-text text-xl" },
          React.createElement("i", { className: `fa-solid ${theme === 'light' ? 'fa-moon' : 'fa-sun'}` })
         ),
        React.createElement("div", { className: "relative" },
          React.createElement("i", { className: "fa-solid fa-globe text-light-text dark:text-dark-text absolute top-1/2 left-3 -translate-y-1/2" }),
          React.createElement("select", {
            ...
            React.createElement("option", { value: "vi" }, "Tiếng Việt")
          )
        ),
        React.createElement("button", { onClick: onApiKeyClick, className: "btn-primary text-white font-bold py-2 px-4 rounded-lg text-sm" }, t('apiKey'))
      )
    )
  );
};
...
const Spinner = () => (
  React.createElement("div", { className: "flex justify-center items-center h-full" },
    React.createElement("div", { className: "animate-spin rounded-full h-16 w-16 border-t-2 border-b-2 border-light-primary dark:border-dark-primary" })
   )
);
...
const ImageUploader = ({ onImageUpload, label }) => {
  ...
  return (
    React.createElement("div", null,
      React.createElement("label", { className: "block text-sm font-bold mb-2 text-light-text dark:text-dark-text" }, label),
      React.createElement("div", {
        ...
        image ? (
          React.createElement("img", { src: image, alt: "Uploaded preview", className: "max-w-full h-auto rounded-md" })
        ) : (
          React.createElement(React.Fragment, null,
            React.createElement("i", { className: "fas fa-cloud-upload-alt text-4xl text-gray-400 mb-2" }),
            React.createElement("p", { className: "text-gray-500 dark:text-gray-400" }, t('dragOrClick'))
          )
        )
      ),
      React.createElement("input", {
...
const ImageComparator = ({ beforeSrc, afterSrc }) => {
    ...
    return (
        React.createElement("div", { ref: containerRef, className: "image-comparator select-none" },
            React.createElement("img", { src: beforeSrc, alt: "Before", className: "comparator-img-before", draggable: "false" }),
            React.createElement("div", { className: "comparator-after", style: { clipPath: `polygon(0 0, ${sliderPosition}% 0, ${sliderPosition}% 100%, 0 100%)` } },
                React.createElement("img", { src: afterSrc, alt: "After", className: "comparator-img-after", draggable: "false" })
            ),
            React.createElement("div", { 
                ...
                React.createElement("div", { className: "comparator-handle" },
                    React.createElement("i", { className: "fas fa-arrows-alt-h text-white" })
                )
            )
        )
    );
...
const ToolCard = ({ icon, title, description, onClick }) => (
  React.createElement("div", {
    ...
    React.createElement("div", { className: "text-2xl text-light-primary dark:text-dark-primary mb-4" },
        React.createElement("i", { className: `fas ${icon}` })
      ),
    React.createElement("h3", { className: "text-xl font-bold mb-2 text-light-text dark:text-dark-text" }, title),
    React.createElement("p", { className: "text-gray-600 dark:text-gray-400" }, description)
  )
);
...
const ToolInfo = ({ toolKey }) => {
    ...
    return (
        React.createElement("div", { className: "mb-6" },
            React.createElement("div", { className: "flex border-b-2 border-light-bg dark:border-dark-bg" },
                tabs.map(tab => (
            ...
            React.createElement("div", { className: "py-4 text-sm text-light-text dark:text-dark-text" },
                React.createElement("ul", { className: "list-disc pl-5 space-y-1" },
                    Array.isArray(currentContent) && currentContent.map((item, index) => React.createElement("li", { key: index }, item))
                )
            )
        )
    );
...
const ToggleSwitch = ({ id, checked, onChange, disabled, label, description }) => (
    React.createElement("div", { className: "flex items-center justify-between bg-light-bg dark:bg-dark-bg p-3 rounded-lg border border-light-border dark:border-dark-border" },
        React.createElement("div", null,
            React.createElement("label", { htmlFor: id, className: "block text-sm font-bold text-light-text dark:text-dark-text cursor-pointer select-none" }, label),
            description && React.createElement("p", { className: "text-xs text-gray-500 select-none" }, description)
        ),
...
const HomePage = ({ onNavigate }) => {
  ...
  return (
    React.createElement("div", { className: "p-8 animate-fade-in" },
      React.createElement("div", { className: "text-center mb-12" },
        React.createElement("h2", { className: "text-5xl md:text-6xl font-extrabold mb-4 bg-gradient-to-r from-fuchsia-500 to-purple-600 bg-clip-text text-transparent" }, t('homeTitle')),
        React.createElement("p", { className: "text-xl text-gray-500 dark:text-gray-300 max-w-2xl mx-auto" }, t('homeSubtitle'))
      ),
...
const PoseStudio = ({ onBack }) => {
    ...
    return (
        React.createElement("div", { className: "p-4 md:p-8 animate-fade-in" },
            React.createElement("button", { onClick: onBack, className: "mb-6 flex items-center text-light-text dark:text-dark-text hover:underline" },
                React.createElement("i", { className: "fas fa-arrow-left mr-2" }), " ", t('goBack')
            ),
            React.createElement("div", { className: "grid grid-cols-1 lg:grid-cols-2 gap-8" },
                React.createElement("div", { className: "space-y-6 bg-light-surface dark:bg-dark-surface p-6 rounded-lg shadow-md border border-light-border dark:border-dark-border" },
                    React.createElement(ToolInfo, { toolKey: "pose" }),
                    React.createElement(ImageUploader, { label: t('uploadCharacter'), onImageUpload: setCharacterImage }),
                    React.createElement(ImageUploader, { label: t('uploadPose'), onImageUpload: setPoseImage }),
                    
                    React.createElement("div", null,
                        React.createElement("label", { className: "block text-sm font-bold mb-2 text-light-text dark:text-dark-text" }, t('preset')),
                        React.createElement("select", { value: preset, onChange: (e) => setPreset(e.target.value), className: "w-full p-2 bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-md text-light-text dark:text-dark-text" },
                            Object.entries(PRESETS).map(([key, value]) => (
                                React.createElement("option", { key: key, value: key }, value.label[language])
                            ))
                        )
                    ),

                     React.createElement("div", null,
                        React.createElement("label", { className: "block text-sm font-bold mb-2 text-light-text dark:text-dark-text" }, t('aspectRatio')),
                        React.createElement("select", { value: aspectRatio, onChange: (e) => setAspectRatio(e.target.value), className: "w-full p-2 bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-md text-light-text dark:text-dark-text" },
                           ASPECT_RATIOS.map(ratio => (React.createElement("option", { key: ratio.value, value: ratio.value }, ratio.label[language])))
                        )
                    ),

                    React.createElement("div", null,
                        React.createElement("label", { className: "block text-sm font-bold mb-2 text-light-text dark:text-dark-text" }, t('outputFormat')),
                        React.createElement("select", { value: outputFormat, onChange: (e) => setOutputFormat(e.target.value), className: "w-full p-2 bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-md text-light-text dark:text-dark-text" },
                    ...
                    React.createElement("div", { className: "flex items-center justify-between" },
                        React.createElement("label", { htmlFor: "beautify-toggle-pose", className: "block text-sm font-bold text-light-text dark:text-dark-text" },
                            t('beautify'),
                            !isBeautyAvailable && React.createElement("span", { className: "text-xs font-normal text-gray-500 ml-2" }, t('beautifyHint'))
                        ),
                        React.createElement("label", { className: "relative inline-flex items-center cursor-pointer" },
                            React.createElement("input", { 
                                type: "checkbox", 
                    ...
                    React.createElement("div", null,
                      React.createElement("label", { className: "block text-sm font-bold mb-2 text-light-text dark:text-dark-text" }, t('controlMode')),
                        React.createElement("div", { className: "flex space-x-2 rounded-lg p-1 bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border" },
                        ...
                        React.createElement("p", { className: "mt-2 text-xs text-center text-gray-500 dark:text-gray-400 min-h-[2.5em] px-1" },
                          t(`controlModeDesc_${controlMode}`)
                        )
                    ),

                    React.createElement("div", null,
                      React.createElement("label", { htmlFor: "positivePrompt", className: "block text-sm font-bold mb-2 text-light-text dark:text-dark-text" }, t('positivePrompt')),
                    ...
                    React.createElement("div", null,
                      React.createElement("label", { htmlFor: "negativePrompt", className: "block text-sm font-bold mb-2 text-light-text dark:text-dark-text" }, t('negativePrompt')),
                      React.createElement("textarea", {
                          id: "negativePrompt",
                ...
                React.createElement("div", { className: "space-y-6" },
                    React.createElement("div", { className: "bg-light-surface dark:bg-dark-surface p-6 rounded-lg shadow-md border border-light-border dark:border-dark-border" },
                         React.createElement("h3", { className: "text-xl font-bold mb-4 text-light-text dark:text-dark-text" }, t('result')),
                         React.createElement("div", { className: "w-full h-96 bg-light-bg dark:bg-dark-bg rounded-lg flex items-center justify-center border border-light-border dark:border-dark-border" },
                             isLoading ? React.createElement(Spinner, null) : resultImage && characterImage ? React.createElement(ImageComparator, { beforeSrc: characterImage.dataUrl, afterSrc: resultImage }) : React.createElement("p", { className: "text-gray-500" }, t('preview'))
                         ),
                         error && React.createElement("p", { className: "text-red-500 mt-4" }, error)
                    ),
                    resultImage && !isLoading && (
                        React.createElement("a", { href: resultImage, download: getDownloadFilename(resultImage), className: "w-full btn-primary text-white font-bold py-3 px-4 rounded-lg flex items-center justify-center" },
                            React.createElement("i", { className: "fas fa-download mr-2" }), " ", t('download')
                        )
                    ),
                    React.createElement("button", { onClick: handleSubmit, disabled: isLoading, className: "w-full btn-primary text-white font-bold py-3 px-4 rounded-lg flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed" },
                        React.createElement("i", { className: "fas fa-cogs mr-2" }), " ", isLoading ? t('generating') : t('generate')
                    )
                )
            )
        )
...
const AIStylist = ({ onBack }) => {
    ...
    return (
        React.createElement("div", { className: "p-4 md:p-8 animate-fade-in" },
            ...
            React.createElement("div", { className: "grid grid-cols-1 lg:grid-cols-2 gap-8" },
                React.createElement("div", { className: "space-y-6 bg-light-surface dark:bg-dark-surface p-6 rounded-lg shadow-md border border-light-border dark:border-dark-border" },
                    React.createElement(ToolInfo, { toolKey: "stylist" }),
                    React.createElement(ImageUploader, { label: t('uploadModel'), onImageUpload: setModelImage }),
                    React.createElement(ImageUploader, { label: t('uploadOutfit'), onImageUpload: setOutfitImage }),
                    
                    React.createElement("div", null,
                        React.createElement("label", { className: "block text-sm font-bold mb-2 text-light-text dark:text-dark-text" }, t('preset')),
                        React.createElement("select", { value: preset, onChange: (e) => setPreset(e.target.value), className: "w-full p-2 bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-md text-light-text dark:text-dark-text" },
                            React.createElement("option", { value: "auto-preset" }, t('autoPreset')),
                            Object.entries(PRESETS).map(([key, value]) => (
                                React.createElement("option", { key: key, value: key }, value.label[language])
                            ))
                        )
                    ...
                    React.createElement("div", { className: "flex items-center justify-between" },
                        React.createElement("label", { htmlFor: "beautify-toggle-stylist", className: "block text-sm font-bold text-light-text dark:text-dark-text" },
                            t('beautify'),
                            !isBeautyAvailable && React.createElement("span", { className: "text-xs font-normal text-gray-500 ml-2" }, t('beautifyHint'))
                        ),
                        React.createElement("label", { className: "relative inline-flex items-center cursor-pointer" },
                    ...
                    React.createElement("div", { className: "bg-light-surface dark:bg-dark-surface p-6 rounded-lg shadow-md border border-light-border dark:border-dark-border" },
                         ...
                         React.createElement("div", { className: "w-full h-96 bg-light-bg dark:bg-dark-bg rounded-lg flex items-center justify-center border border-light-border dark:border-dark-border" },
                            isLoading ? (
                                ...
                                React.createElement("img", { src: resultImage, alt: t('result'), className: "object-contain max-h-full max-w-full rounded-lg" })
                            ) : (
                                React.createElement("p", { className: "text-gray-500" }, t('preview'))
                            )
                         ),
                         error && React.createElement("p", { className: "text-red-500 mt-4" }, error)
                    ),
...
const PropFusion = ({ onBack }) => {
    ...
    return (
        React.createElement("div", { className: "p-4 md:p-8 animate-fade-in" },
            React.createElement("button", { onClick: onBack, className: "mb-6 flex items-center text-light-text dark:text-dark-text hover:underline" }, React.createElement("i", { className: "fas fa-arrow-left mr-2" }), " ", t('goBack')),
            React.createElement("div", { className: "grid grid-cols-1 lg:grid-cols-2 gap-8" },
                React.createElement("div", { className: "space-y-6 bg-light-surface dark:bg-dark-surface p-6 rounded-lg shadow-md border border-light-border dark:border-dark-border" },
                    React.createElement(ToolInfo, { toolKey: "prop" }),
                    React.createElement(ImageUploader, { label: t('uploadCharacter'), onImageUpload: setCharacterImage }),
                    React.createElement(ImageUploader, { label: t('uploadProp'), onImageUpload: setPropImage }),
                    React.createElement("div", null,
                        React.createElement("label", { className: "block text-sm font-bold mb-2 text-light-text dark:text-dark-text" }, t('preset')),
                        React.createElement("select", { value: preset, onChange: (e) => setPreset(e.target.value), className: "w-full p-2 bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-md text-light-text dark:text-dark-text" },
                            Object.entries(PRESETS).map(([key, value]) => (React.createElement("option", { key: key, value: key }, value.label[language])))
                        )
                    ),
                    ...
                    React.createElement("div", { className: "flex items-center justify-between" },
                        React.createElement("label", { htmlFor: "beautify-toggle-prop", className: "block text-sm font-bold text-light-text dark:text-dark-text" },
                            t('beautify'),
                            !isBeautyAvailable && React.createElement("span", { className: "text-xs font-normal text-gray-500 ml-2" }, t('beautifyHint'))
                        ),
                        React.createElement("label", { className: "relative inline-flex items-center cursor-pointer" },
                    ...
                    React.createElement("div", null,
                      React.createElement("label", { htmlFor: "positivePrompt", className: "block text-sm font-bold mb-2 text-light-text dark:text-dark-text" }, t('positivePrompt')),
                      React.createElement("textarea", { id: "positivePrompt", value: positivePrompt, onChange: (e) => setPositivePrompt(e.target.value), placeholder: t('positivePromptPlaceholder'), rows: 3, className: "w-full p-2 bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-md text-light-text dark:text-dark-text" })
                    ),
                    React.createElement("div", null,
                      React.createElement("label", { htmlFor: "negativePrompt", className: "block text-sm font-bold mb-2 text-light-text dark:text-dark-text" }, t('negativePrompt')),
                      React.createElement("textarea", { id: "negativePrompt", value: negativePrompt, onChange: (e) => setNegativePrompt(e.target.value), placeholder: t('negativePromptPlaceholder'), rows: 3, className: "w-full p-2 bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-md text-light-text dark:text-dark-text" })
                    )
                ),
                React.createElement("div", { className: "space-y-6" },
                    React.createElement("div", { className: "bg-light-surface dark:bg-dark-surface p-6 rounded-lg shadow-md border border-light-border dark:border-dark-border" },
...
const AIDesign = ({ onBack }) => {
    ...
    return (
        React.createElement("div", { className: "p-4 md:p-8 animate-fade-in" },
            ...
            React.createElement("div", { className: "grid grid-cols-1 lg:grid-cols-2 gap-8" },
                React.createElement("div", { className: "space-y-6 bg-light-surface dark:bg-dark-surface p-6 rounded-lg shadow-md border border-light-border dark:border-dark-border" },
                    React.createElement(ToolInfo, { toolKey: "design" }),
                    React.createElement(ImageUploader, { label: t('uploadSubject'), onImageUpload: setSubjectImage }),
                    React.createElement(ImageUploader, { label: t('uploadBackground'), onImageUpload: setBackgroundImage }),
                    React.createElement("div", null,
                        React.createElement("label", { className: "block text-sm font-bold mb-2 text-light-text dark:text-dark-text" }, t('preset')),
                        React.createElement("select", { value: preset, onChange: (e) => setPreset(e.target.value), className: "w-full p-2 bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-md text-light-text dark:text-dark-text" },
                            Object.entries(PRESETS).map(([key, value]) => (React.createElement("option", { key: key, value: key }, value.label[language])))
                    ...
                    React.createElement("div", { className: "flex items-center justify-between" },
                        React.createElement("label", { htmlFor: "beautify-toggle-design", className: "block text-sm font-bold text-light-text dark:text-dark-text" },
                            t('beautify'),
                            !isBeautyAvailable && React.createElement("span", { className: "text-xs font-normal text-gray-500 ml-2" }, t('beautifyHint'))
                        ),
                        React.createElement("label", { className: "relative inline-flex items-center cursor-pointer" },
                ...
                React.createElement("div", { className: "space-y-6" },
                    React.createElement("div", { className: "bg-light-surface dark:bg-dark-surface p-6 rounded-lg shadow-md border border-light-border dark:border-dark-border" },
                         React.createElement("h3", { className: "text-xl font-bold mb-4 text-light-text dark:text-dark-text" }, t('result')),
                         React.createElement("div", { className: "w-full h-96 bg-light-bg dark:bg-dark-bg rounded-lg flex items-center justify-center border border-light-border dark:border-dark-border" },
                             isLoading ? React.createElement(Spinner, null) : resultImage && subjectImage ? React.createElement(ImageComparator, { beforeSrc: subjectImage.dataUrl, afterSrc: resultImage }) : React.createElement("p", { className: "text-gray-500" }, t('preview'))
                         ),
                         error && React.createElement("p", { className: "text-red-500 mt-4" }, error)
                    ),
                    resultImage && !isLoading && (
...
const AICreative = ({ onBack }) => {
    ...
    return (
        React.createElement("div", { className: "p-4 md:p-8 animate-fade-in" },
            ...
            React.createElement("div", { className: "grid grid-cols-1 lg:grid-cols-2 gap-8" },
                React.createElement("div", { className: "space-y-6 bg-light-surface dark:bg-dark-surface p-6 rounded-lg shadow-md border border-light-border dark:border-dark-border" },
                    React.createElement(ToolInfo, { toolKey: "creative" }),
                    React.createElement("div", null,
                        React.createElement("label", { htmlFor: "positivePrompt", className: "block text-sm font-bold mb-2 text-light-text dark:text-dark-text" }, t('imagePrompt')),
                        React.createElement("textarea", { id: "positivePrompt", value: positivePrompt, onChange: (e) => setPositivePrompt(e.target.value), placeholder: t('imagePromptPlaceholder'), rows: 5, className: "w-full p-2 bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-md text-light-text dark:text-dark-text" })
                    ),
                    React.createElement("div", null,
                        React.createElement("label", { className: "block text-sm font-bold mb-2 text-light-text dark:text-dark-text" }, t('preset')),
                        React.createElement("select", { value: preset, onChange: (e) => setPreset(e.target.value), className: "w-full p-2 bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-md text-light-text dark:text-dark-text" },
                     ...
                     React.createElement("div", null,
                        React.createElement("label", { className: "block text-sm font-bold mb-2 text-light-text dark:text-dark-text" }, t('aspectRatio')),
                        React.createElement("select", { value: aspectRatio, onChange: (e) => setAspectRatio(e.target.value), className: "w-full p-2 bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-md text-light-text dark:text-dark-text" },
                           ASPECT_RATIOS.filter(r => r.value !== 'auto').map(ratio => (React.createElement("option", { key: ratio.value, value: ratio.value }, ratio.label[language])))
                        )
                    ),
                    React.createElement("div", null,
                        React.createElement("label", { className: "block text-sm font-bold mb-2 text-light-text dark:text-dark-text" }, t('outputFormat')),
                    ...
                    React.createElement("div", { className: "flex items-center justify-between" },
                        React.createElement("label", { htmlFor: "beautify-toggle-creative", className: "block text-sm font-bold text-light-text dark:text-dark-text" },
                            t('beautify'),
                             !isBeautyAvailable && React.createElement("span", { className: "text-xs font-normal text-gray-500 ml-2" }, t('beautifyHint'))
                        ),
                        React.createElement("label", { className: "relative inline-flex items-center cursor-pointer" }, React.createElement("input", { type: "checkbox", id: "beautify-toggle-creative", checked: beauty === 'on', onChange: (e) => setBeauty(e.target.checked ? 'on' : 'off'), className: "sr-only peer", disabled: !isBeautyAvailable }),
                            React.createElement("div", { className: "w-11 h-6 bg-gray-200 rounded-full peer peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-light-primary/50 dark:peer-focus:ring-dark-primary/50 dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-light-primary dark:peer-checked:bg-dark-primary" })
                        )
                    ),
                    React.createElement("div", null,
                ...
                React.createElement("div", { className: "space-y-6" },
                    React.createElement("div", { className: "bg-light-surface dark:bg-dark-surface p-6 rounded-lg shadow-md border border-light-border dark:border-dark-border" },
                         React.createElement("h3", { className: "text-xl font-bold mb-4 text-light-text dark:text-dark-text" }, t('result')),
                         React.createElement("div", { className: "w-full h-96 bg-light-bg dark:bg-dark-bg rounded-lg flex items-center justify-center border border-light-border dark:border-dark-border" },
                             isLoading ? React.createElement(Spinner, null) : resultImage ? React.createElement("img", { src: resultImage, alt: "Generated result", className: "object-contain max-h-full max-w-full rounded-lg" }) : React.createElement("p", { className: "text-gray-500" }, t('preview'))
                         ),
                         error && React.createElement("p", { className: "text-red-500 mt-4" }, error)
                    ),
                    resultImage && !isLoading && (
...
const AIArchitect = ({ onBack }) => {
    ...
    return (
        React.createElement("div", { className: "p-4 md:p-8 animate-fade-in" },
            ...
            React.createElement("div", { className: "grid grid-cols-1 lg:grid-cols-2 gap-8" },
                React.createElement("div", { className: "space-y-6 bg-light-surface dark:bg-dark-surface p-6 rounded-lg shadow-md border border-light-border dark:border-dark-border" },
                    React.createElement(ToolInfo, { toolKey: "architect" }),
                    React.createElement(ImageUploader, { label: t('uploadBlueprint'), onImageUpload: setBlueprintImage }),
                    React.createElement("div", null,
                        React.createElement("label", { className: "block text-sm font-bold mb-2 text-light-text dark:text-dark-text" }, t('preset')),
                        React.createElement("select", { value: preset, onChange: (e) => setPreset(e.target.value), className: "w-full p-2 bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border rounded-md text-light-text dark:text-dark-text" },
                            Object.entries(PRESETS).filter(([key]) => key.startsWith('architecture-')).map(([key, value]) => (React.createElement("option", { key: key, value: key }, value.label[language])))
                        )
                    ),
                     React.createElement("div", null,
                        React.createElement("label", { className: "block text-sm font-bold mb-2 text-light-text dark:text-dark-text" }, t('aspectRatio')),
                ...
                React.createElement("div", { className: "space-y-6" },
                    React.createElement("div", { className: "bg-light-surface dark:bg-dark-surface p-6 rounded-lg shadow-md border border-light-border dark:border-dark-border" },
                         React.createElement("h3", { className: "text-xl font-bold mb-4 text-light-text dark:text-dark-text" }, t('result')),
                         React.createElement("div", { className: "w-full h-96 bg-light-bg dark:bg-dark-bg rounded-lg flex items-center justify-center border border-light-border dark:border-dark-border" },
                             isLoading ? React.createElement(Spinner, null) : resultImage && blueprintImage ? React.createElement(ImageComparator, { beforeSrc: blueprintImage.dataUrl, afterSrc: resultImage }) : React.createElement("p", { className: "text-gray-500" }, t('preview'))
                         ),
                         error && React.createElement("p", { className: "text-red-500 mt-4" }, error)
                    ),
                    resultImage && !isLoading && (
    ...
    return (
        React.createElement("div", { className: "p-4 md:p-8 animate-fade-in" },
            ...
            React.createElement("div", { className: "grid grid-cols-1 lg:grid-cols-2 gap-8" },
                React.createElement("div", { className: "space-y-6 bg-light-surface dark:bg-dark-surface p-6 rounded-lg shadow-md border border-light-border dark:border-dark-border" },
                    React.createElement("div", null,
                        React.createElement("label", { className: "block text-sm font-bold mb-2 text-light-text dark:text-dark-text" }, t('generationMode')),
                        React.createElement("div", { className: "flex space-x-2 rounded-lg p-1 bg-light-bg dark:bg-dark-bg border border-light-border dark:border-dark-border" },
                            React.createElement("button", { onClick: () => setMode('text'), className: `w-full px-3 py-2 text-sm font-semibold rounded-md transition-colors duration-200 ${mode === 'text' ? 'btn-primary text-white' : 'bg-transparent text-light-text dark:text-dark-text'}` },
                                t('textToVideo')
                            ),
                            React.createElement("button", { onClick: () => setMode('image'), className: `w-full px-3 py-2 text-sm font-semibold rounded-md transition-colors duration-200 ${mode === 'image' ? 'btn-primary text-white' : 'bg-transparent text-light-text dark:text-dark-text'}` },
                                t('imageToVideo')
                            )
                        )
                    ),
                    ...
                    mode === 'image' && (
                        React.createElement(React.Fragment, null,
                           React.createElement(ImageUploader, { label: t('uploadCharacter'), onImageUpload: setImage }),
                           React.createElement(ImageUploader, { label: t('uploadContext'), onImageUpload: setContextImage })
                        )
                    ),

                    React.createElement("div", null,
                      React.createElement("label", { htmlFor: "videoPrompt", className: "block text-sm font-bold mb-2 text-light-text dark:text-dark-text" }, t('videoPrompt')),
                      React.createElement("textarea", {
                          id: "videoPrompt",
                          value: prompt,
                    ...
                    React.createElement("div", null,
                        React.createElement("label", { className: "block text-sm font-bold mb-2 text-light-text dark:text-dark-text" }, t('videoAspectRatio')),
                        React.createElement("select", {
                            value: aspectRatio,
                            ...
                            React.createElement("option", { value: "16:9" }, t('landscape')),
                            React.createElement("option", { value: "9:16" }, t('portrait')),
                            React.createElement("option", { value: "1:1" }, t('videoAspectSquare'))
                        )
                    ),

                    React.createElement("div", null,
                        React.createElement("label", { className: "block text-sm font-bold mb-2 text-light-text dark:text-dark-text" }, t('videoQuality')),
                        React.createElement("select", {
                            value: quality,
                            ...
                            React.createElement("option", { value: "720p" }, t('hd720')),
                            React.createElement("option", { value: "1080p" }, t('hd1080'))
                        )
                    )
                ...
                React.createElement("div", { className: "space-y-6" },
                    React.createElement("div", { className: "bg-light-surface dark:bg-dark-surface p-6 rounded-lg shadow-md border border-light-border dark:border-dark-border" },
                         React.createElement("h3", { className: "text-xl font-bold mb-4 text-light-text dark:text-dark-text" }, t('videoResult')),
                         React.createElement("div", { className: "w-full aspect-video bg-light-bg dark:bg-dark-bg rounded-lg flex items-center justify-center border border-light-border dark:border-dark-border" },
                            isLoading ? (
                                React.createElement("div", { className: "text-center p-4" },
                                    React.createElement(Spinner, null),
                                    React.createElement("p", { className: "mt-4 font-semibold text-light-text dark:text-dark-text" }, loadingMessage),
                                    React.createElement("p", { className: "text-sm text-gray-500" }, t('videoTakesTime'))
                                )
                             ) : resultVideo ? (
                                 React.createElement("video", { src: resultVideo, controls: true, muted: true, playsInline: true, className: "w-full h-full rounded-lg" })
                             ) : (
                                React.createElement("p", { className: "text-gray-500" }, t('videoWillAppear'))
                             )
                         ),
                         error && React.createElement("p", { className: "text-red-500 mt-4 text-center" }, error)
                    ),
                    resultVideo && !isLoading && (
                        React.createElement("a", { href: resultVideo, download: "generated-video.mp4", className: "w-full btn-primary text-white font-bold py-3 px-4 rounded-lg flex items-center justify-center" },
                            React.createElement("i", { className: "fas fa-download mr-2" }), " ", t('download')
                        )
                    ),
                    React.createElement("button", { onClick: handleSubmit, disabled: isLoading, className: "w-full btn-primary text-white font-bold py-3 px-4 rounded-lg flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed" },
                        React.createElement("i", { className: "fas fa-film mr-2" }), " ", isLoading ? t('generatingVideo') : t('generateVideo')
                    )
                )
            )
        )
...
const AIMagic = ({ onBack }) => {
    ...
    return (
        React.createElement("div", { className: "p-4 md:p-8 animate-fade-in" },
            ...
            React.createElement("div", { className: "grid grid-cols-1 lg:grid-cols-2 gap-8" },
                React.createElement("div", { className: "space-y-6 bg-light-surface dark:bg-dark-surface p-6 rounded-lg shadow-md border border-light-border dark:border-dark-border" },
                    React.createElement(ToolInfo, { toolKey: "magic" }),
                    React.createElement(ImageUploader, { label: t('uploadSourceImage'), onImageUpload: setSourceImage }),
                    
                    React.createElement("div", null,
                        React.createElement("label", { htmlFor: "magic-positive-prompt", className: "block text-sm font-bold mb-2 text-light-text dark:text-dark-text" }, t('positivePrompt')),
                        React.createElement("textarea", {
                    ...
                    React.createElement("div", null,
                        React.createElement("label", { htmlFor: "magic-negative-prompt", className: "block text-sm font-bold mb-2 text-light-text dark:text-dark-text" }, t('negativePrompt')),
                        React.createElement("textarea", {
                    ...
                    React.createElement("div", { className: "space-y-4" },
                        ...
                        React.createElement("hr", { className: "border-light-border dark:border-dark-border" }),
                         React.createElement(ToggleSwitch, {
                            id: "magic-preserve",
                ...
                React.createElement("div", { className: "space-y-6" },
                    React.createElement("div", { className: "bg-light-surface dark:bg-dark-surface p-6 rounded-lg shadow-md border border-light-border dark:border-dark-border" },
                         React.createElement("h3", { className: "text-xl font-bold mb-4 text-light-text dark:text-dark-text" }, t('result')),
                         React.createElement("div", { className: "w-full h-96 bg-light-bg dark:bg-dark-bg rounded-lg flex items-center justify-center border border-light-border dark:border-dark-border" },
                             isLoading ? React.createElement(Spinner, null) : resultImage && sourceImage ? React.createElement(ImageComparator, { beforeSrc: sourceImage.dataUrl, afterSrc: resultImage }) : React.createElement("p", { className: "text-gray-500" }, t('preview'))
                         ),
                         error && React.createElement("p", { className: "text-red-500 mt-4 text-center" }, error)
                    ),
                    resultImage && !isLoading && (
                    ...
                    React.createElement("button", { onClick: handleSubmit, disabled: isLoading, className: "w-full btn-primary text-white font-bold py-3 px-4 rounded-lg flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed" },
                        React.createElement("i", { className: "fas fa-wand-magic-sparkles mr-2" }), " ", isLoading ? t('generating') : t('generate')
                    )
                )
            )
        )
...
const AIBackground = ({ onBack }) => {
    ...
    return (
        React.createElement("div", { className: "p-4 md:p-8 animate-fade-in" },
            ...
            React.createElement("div", { className: "grid grid-cols-1 lg:grid-cols-2 gap-8" },
                React.createElement("div", { className: "space-y-6 bg-light-surface dark:bg-dark-surface p-6 rounded-lg shadow-md border border-light-border dark:border-dark-border" },
                    React.createElement(ToolInfo, { toolKey: "background" }),
                    React.createElement(ImageUploader, { label: t('uploadImage'), onImageUpload: setSourceImage })
                ),
                React.createElement("div", { className: "space-y-6" },
                    React.createElement("div", { className: "bg-light-surface dark:bg-dark-surface p-6 rounded-lg shadow-md border border-light-border dark:border-dark-border" },
                         React.createElement("h3", { className: "text-xl font-bold mb-4 text-light-text dark:text-dark-text" }, t('result')),
                         React.createElement("div", { className: "w-full h-96 bg-light-bg dark:bg-dark-bg rounded-lg flex items-center justify-center border border-light-border dark:border-dark-border checkerboard-bg" },
                             isLoading ? React.createElement(Spinner, null) : resultImage && sourceImage ? React.createElement(ImageComparator, { beforeSrc: sourceImage.dataUrl, afterSrc: resultImage }) : React.createElement("p", { className: "text-gray-500" }, t('preview'))
                         ),
                         error && React.createElement("p", { className: "text-red-500 mt-4 text-center" }, error)
                    ),
                    ...
                    React.createElement("button", { onClick: handleSubmit, disabled: isLoading || !sourceImage, className: "w-full btn-primary text-white font-bold py-3 px-4 rounded-lg flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed" },
                        React.createElement("i", { className: "fas fa-eraser mr-2" }), " ", isLoading ? t('generating') : t('generate')
                    )
                )
            )
        )
...
const Footer = () => {
  return (
    React.createElement("footer", { className: "bg-light-surface dark:bg-dark-surface p-4 text-center border-t border-light-border dark:border-dark-border" },
...
const App = () => {
  ...
  const renderPage = () => {
    switch (page) {
      case 'pose':
        return React.createElement(PoseStudio, { onBack: () => setPage('home') });
      case 'prop':
        return React.createElement(PropFusion, { onBack: () => setPage('home') });
      case 'design':
        return React.createElement(AIDesign, { onBack: () => setPage('home') });
      case 'creative':
        return React.createElement(AICreative, { onBack: () => setPage('home') });
      case 'stylist':
        return React.createElement(AIStylist, { onBack: () => setPage('home') });
      case 'architect':
        return React.createElement(AIArchitect, { onBack: () => setPage('home') });
      case 'video':
        return React.createElement(AIVideoCreator, { onBack: () => setPage('home') });
      case 'magic':
        return React.createElement(AIMagic, { onBack: () => setPage('home') });
      case 'background':
        return React.createElement(AIBackground, { onBack: () => setPage('home') });
      case 'home':
      default:
        return React.createElement(HomePage, { onNavigate: handleNavigate });
    }
  };

  return (
    React.createElement(AppContext.Provider, { value: contextValue },
      React.createElement("div", { className: `min-h-screen flex flex-col bg-light-bg dark:bg-dark-bg text-light-text dark:text-dark-text transition-colors duration-300` },
        React.createElement(Header, { onApiKeyClick: () => setApiKeyModalOpen(true) }),
        React.createElement("main", { className: "flex-grow" }, renderPage()),
        React.createElement(Footer, null),
        React.createElement(ApiKeyModal, { isOpen: isApiKeyModalOpen, onClose: () => setApiKeyModalOpen(false) })
      )
    )
  );
};
